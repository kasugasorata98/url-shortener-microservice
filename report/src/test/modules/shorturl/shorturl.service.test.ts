...
describe('ShortURLService', () => {
  ...
  describe('addToDb', () => {
    it('should add a new document to the database', async () => {
      ...
      const titleTag = 'Example Website'
      await service.addToDb(shortId, targetUrl, titleTag)

      const document = await shorturlModel.findOne({ shortId })

      expect(document).toBeDefined()
      expect(document?.targetUrl).toBe(targetUrl)
      expect(document?.titleTag).toBe(titleTag)
    })
  })
})
